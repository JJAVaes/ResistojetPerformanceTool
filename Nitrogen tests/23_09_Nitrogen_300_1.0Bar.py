...
def find_valve_open_times(time_valve_array, valve_array):
    ...
    n = 0
    for i in range(len(valve_array) - 1):
        if time_valve_array[i] > n * 600 + 60 and time_valve_array[i+1] < n * 600 + 61:
            valve_open_start_times.append(time_valve_array[i])
        elif time_valve_array[i] > n * 600 + 360 and time_valve_array[i+1] < n * 600 + 361:
            valve_open_end_times.append(time_valve_array[i])
            n += 1
    return valve_open_start_times, valve_open_end_times

def compute_cut_indexes(time_array, data_array, start_time, end_time, startup_period=20.0, shutdown_period=3.0, custom_startup_period=None):
...
# Define the maximum time (in seconds) for cropping
max_time_seconds = 40.9 * 60  # 41 minutes in seconds

    # Crop based on sampling rates
def crop_data(time_array, data_array, sampling_rate):
    max_points = int(max_time_seconds * sampling_rate)
    return time_array[:max_points], data_array[:max_points]

def process_results(result):
    # testname = 'H2O Test 400°C 1.0 Bar:'
    Time_valve_array = result.loc[9, 'x'] #Based on valve open close other charachteristics can be determined
    Valve_array = result.loc[9, 'y']
    # plot_valveposition(Time_valve_array, Valve_array, testname)

    time_load_array =  result.loc[12, 'x']  # Assuming 'x' column represents time
    load_thruster_array = result.loc[12, 'y']* (1 / 0.02596344) * (25 / 187.5)  # Assuming 'y' column represents load data
    # plot_load(time_load_array, load_thruster_array, testname)

    density = 1.1648 #[kg/m^3] == [mg/cm^3] NIST 
    time_massflow_array = result.loc[0, 'x']
    massflow_array = result.loc[0, 'y'] *density / 60 #[sccm] to [mg/s]
    ...
    time_power_array = result.loc[3, 'x']
    power_array = result.loc[3, 'y'] + result.loc[7, 'y']
    filtered_power_array = filter_data_savgol(power_array)
    # plot_power(time_power_array, power_array, filtered_power_array, testname)

    time_pressureTC_array = result.loc[13, 'x']
    pressureTC_array = result.loc[13, 'y']  #[sccm]
    ...
    time_temperaturePS_array = result.loc[14, 'x']
    temperaturePS_array = result.loc[14, 'y']  #[sccm]
    # plot_temperature(time_temperaturePS_array, temperaturePS_array, 'at interface', testname)
    
    time_pressurevacuum_array = result.loc[15, 'x']
    pressurevacuum_array = result.loc[15, 'y']  #
    # plot_vacuumpressure(time_pressurevacuum_array, pressurevacuum_array, testname)
    
    Time_temperature_array = result.loc[16, 'x']  #
    Temperature1_array =  result.loc[16, 'y']  # Temperature thruster
    Temperature2_array =  result.loc[17, 'y']  # Temperature bottom propellant line
    Temperature3_array =  result.loc[18, 'y']  # Temperature heater 1 (bottom)

    # Valve position (2 Hz)
    Time_valve_array, Valve_array = crop_data(Time_valve_array, Valve_array, 2)
    plot_valveposition(Time_valve_array, Valve_array, testname)

    # Load Thruster (30 Hz)
    time_load_array, load_thruster_array = crop_data(time_load_array, load_thruster_array, 30)
    plot_load(time_load_array, load_thruster_array, testname)

    # Mass Flow (25 Hz)
    time_massflow_array, massflow_array = crop_data(time_massflow_array, massflow_array, 25)
    filtered_massflow_array = filter_data_savgol(massflow_array)
    plot_massflow(time_massflow_array, massflow_array, filtered_massflow_array, testname)

    # Power (25 Hz)
    time_power_array, power_array = crop_data(time_power_array, power_array, 25)
    filtered_power_array = filter_data_savgol(power_array)
    plot_power(time_power_array, power_array, filtered_power_array, testname)

    # Pressure TC (10 Hz)
    time_pressureTC_array, pressureTC_array = crop_data(time_pressureTC_array, pressureTC_array, 10)
    filtered_pressure_array = filter_data_savgol(pressureTC_array)
    plot_pressure(time_pressureTC_array, pressureTC_array, filtered_pressure_array, testname)

    # Temperature PS (10 Hz)
    time_temperaturePS_array, temperaturePS_array = crop_data(time_temperaturePS_array, temperaturePS_array, 10)
    plot_temperature(time_temperaturePS_array, temperaturePS_array, 'at interface', testname)

    # Vacuum Pressure (10 Hz)
    time_pressurevacuum_array, pressurevacuum_array = crop_data(time_pressurevacuum_array, pressurevacuum_array, 10)
    plot_vacuumpressure(time_pressurevacuum_array, pressurevacuum_array, testname)

    # Temperatures (2 Hz)
    Time_temperature_array, Temperature1_array = crop_data(Time_temperature_array, Temperature1_array, 2)
    _, Temperature2_array = crop_data(Time_temperature_array, Temperature2_array, 2)
    _, Temperature3_array = crop_data(Time_temperature_array, Temperature3_array, 2)

    # You can plot the temperatures similarly as above, depending on how you wish to visualize them.
    ...
    plt.figure()
    plt.plot(time_load_array/60, load_thruster_array, label='Thrust (Unfiltered)')
    #plt.plot(time_load_array/60, filtered_loads_data_butterworth, label='Thrust (Butterworth Filter)')
    plt.plot(time_load_array/60, filtered_loads_data_savgol, label='Thrust (Filtered)')
    plt.xlabel('Time [min]')
    plt.ylabel('Thrust [mN]')
    ...
    plot_MP(time_massflow_array, massflow_array, time_pressureTC_array, filtered_pressure_array, testname)
    plot_MT(time_massflow_array, massflow_array, time_load_array, loads_zero_drift, testname)
    ...
    for i, (start_time, end_time) in enumerate(zip(valve_open_start_times, valve_open_end_times)):
        ...
        cut_massflow_data, mean_massflow, sigma_massflow = compute_cut_indexes(time_massflow_array, massflow_array, start_time, end_time)
        ...
        # Print results
        print(f"NI-2.{test_number}:")
        print(f"NI-2.{test_number} Thrust [mN]: {mean_thrust:.3f} \u00B1 {sigma_thrust:.3f} ({sigma_thrust/mean_thrust*100:.3f})%")
        print(f"NI-2.{test_number} Massflow [mg/s]: {mean_massflow:.3f} \u00B1 {sigma_massflow:.3f}({sigma_massflow/mean_massflow*100:.3f})%")
        print(f"NI-2.{test_number} Pressure [mbar]: {mean_pressure:.3f} \u00B1 {sigma_pressure:.3f}({sigma_pressure/mean_pressure*100:.3f})%")
        print(f"NI-2.{test_number} Temperature of the thruster [C]: {mean_temperaturethruster:.3f} \u00B1 {sigma_temperaturethruster:.3f}({sigma_temperaturethruster/mean_temperaturethruster*100:.3f})%")
        print(f"NI-2.{test_number} Heating power [W]: {mean_power:.1f} \u00B1 {sigma_power:.1f}({sigma_power/mean_power*100:.1f})%")
        print(f"NI-2.{test_number} Ambient temperature [C]: {mean_temperatureambient:.1f} \u00B1 {sigma_temperatureambient:.1f}({sigma_temperatureambient/mean_temperatureambient*100:.1f})%")
        print(f"NI-2.{test_number} Ambient pressure [mbar]: {mean_pressureambient:.1f} \u00B1 {sigma_pressureambient:.1f}({sigma_pressureambient/mean_pressureambient*100:.1f})%")
...
result = read_tdms(r"Nitrogen tests\20240923 N2_300_1.0Bar\171001.tdms")
